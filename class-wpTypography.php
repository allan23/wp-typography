...
*/
		...
		// Clear the object cache for a post when updated.
...
*/
	...
	function process($text, $isTitle = FALSE) {
		...
		if(is_feed()) { //feed readers can be pretty stupid
			...
			global $post;
			$key		 = $post->ID . '_' . (int) $isTitle;
			$the_post	 = wp_cache_get( $key );
			if ( false === $the_post ) {
				$the_post = $this->phpTypo->process( $text, $isTitle );
				wp_cache_set( $key, $the_post );
			}
			return $the_post;
		}
	}
...
<?php
	...
	/**
	 * Clears the object cache when a post is updated.
